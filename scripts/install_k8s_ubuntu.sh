...
install_docker_and_kubelet() {
  apt-get update && apt-get install -y apt-transport-https ca-certificates curl software-properties-common ipvsadm
...
#  apt-get update
  ...
  sudo add-apt-repository "deb [arch=amd64] https://mirrors.aliyun.com/docker-ce/linux/ubuntu focal test"
#  sudo apt-get -y update
#  Use systemd driver instead of cgroupfs driver.
  distribution=$(. /etc/os-release;echo $ID$VERSION_ID)
  curl -s -L https://nvidia.github.io/nvidia-docker/gpgkey | sudo apt-key add -
  curl -s -L https://nvidia.github.io/nvidia-docker/$distribution/nvidia-docker.list | sudo tee /etc/apt/sources.list.d/nvidia-docker.list
# systemd is still not usable
# "exec-opts": ["native.cgroupdriver=systemd"]
  sudo apt-get update
  sudo apt-get install -y docker-ce nvidia-docker2 kubelet kubeadm kubectl
...
setup_nvidia_environment()  {
  ...
  sed -i "s/^\# accept-nvidia-visible-devices-envvar-when-unprivileged = true/accept-nvidia-visible-devices-envvar-when-unprivileged = false/g" /etc/nvidia-container-runtime/config.toml
  sed -i "s/^\# accept-nvidia-visible-devices-as-volume-mounts = true/accept-nvidia-visible-devices-as-volume-mounts = false/g" /etc/nvidia-container-runtime/config.toml
